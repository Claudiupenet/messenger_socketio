...
const send_friend_request = (req, res) => {
	if (!req.body.friend_id) {
		...
		//User.findOne({email: req.user.email, "friends.email": email_friend})
		User.findById(req.body.friend_id)
			.then(data => {
				User.updateOne({ email: data.email }, {
					$push: {
						friends: { email: req.user.email, status: 1 }
					}
				})
					.then(() => {
						User.updateOne({ email: req.user.email }, {
							$push: {
								friends: { email: data.email, status: 2 }
							}
						})
							.then(() => {
			...
			.catch(() => {
				res.status(404).json({ message: "Wrong friend id!" })
			})
	}
}
...
const confirm_friend_request = (req, res) => {
	if (!req.body.friend_id) {
		...
		User.findById(req.body.friend_id)
			.then(friend => {
				User.updateOne({ email: friend.email, "friends.email": req.user.email }, {
					$set: {
						"friends.$.status": 0
					}
				})
					.then(() => {
						User.updateOne({ email: req.user.email, "friends.email": friend.email },
							{
								$set: {
									"friends.$.status": 0
								}
							...
							.catch(() => {
								res.status(500).json({ message: "Database error at confirm friend request!" })
							})
					})
					.catch((e) => {
						console.log(e)
